...
export default function CoordEvents() {
  ...
  /* ── Stage schedule (estimated date/venue for later auto-advanced rounds) ── */
  const [stageSchedule,       setStageSchedule]       = useState([]); // [{label,date,time,venue}]
  const [stageScheduleSaving, setStageScheduleSaving] = useState(false);
  ...
  const loadStageSchedule = (eventId) => {
    api.get(`/events/${eventId}/stage-schedule`)
      .then(d => setStageSchedule(d.stages || []))
      .catch(() => setStageSchedule([]));
  };
  ...
  /* ── Stage schedule (estimated date/venue per later auto-advanced round) ── */
  const updateStageField = (label, key, val) =>
    setStageSchedule(prev => prev.map(s => s.label === label ? { ...s, [key]: val } : s));

  const handleSaveStageSchedule = async () => {
    setStageScheduleSaving(true);
    try {
      await api.put(`/events/${regEvent._id}/stage-schedule`, { stages: stageSchedule });
      showToast('Stage dates saved.');
    } catch (err) {
      showToast(err.message || 'Failed to save stage dates.', 'err');
    } finally {
      setStageScheduleSaving(false);
    }
  };
                      ...
                      {stageSchedule.map(stage => (
                        <div key={stage.label} className={es.fixtureMatchLine}>
                          ...
                          <input
                            ...
                            className={es.fixtureInput}
                            value={stage.date}
                            onChange={e => updateStageField(stage.label, 'date', e.target.value)} />
                          <input
                            className={es.fixtureInput}
                            style={{ maxWidth: 110 }}
                            type="text"
                            placeholder="Time e.g. 4:00 PM"
                            value={stage.time}
                            onChange={e => updateStageField(stage.label, 'time', e.target.value)} />
                          <input
                            className={es.fixtureInput}
                            placeholder="Venue"
                            value={stage.venue}
                            onChange={e => updateStageField(stage.label, 'venue', e.target.value)} />
                        </div>
                      ))}
                    </div>
                    <div className={es.fixtureActions}>
                      <button
                        ...
                        disabled={stageScheduleSaving}>
                        {stageScheduleSaving ? 'Saving…' : 'Save Stage Dates'}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className={es.reportPlaceholder}>
                    ...
                    <button
                      ...
                      <>
                        ...
                        {(() => {
                          const w = eventReport.summary_stats?.tournamentWinner;
                          if (!w) return null;
