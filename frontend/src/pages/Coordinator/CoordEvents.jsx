...
export default function CoordEvents() {
  ...
  /* ── Fixtures state ── */
  const [fixturesByDiv,    setFixturesByDiv]    = useState({ boys: [], girls: [] }); // date-grouped for editing UI, per division
  const [flatFixtures,     setFlatFixtures]     = useState([]); // flat list with IDs + results, both divisions tagged
  const [fixturesDeclared, setFixturesDeclared] = useState(false);
  const [declareLoading,   setDeclareLoading]   = useState({ boys: false, girls: false });

  /* ── Stage schedule (estimated date/venue for later auto-advanced rounds), per division ── */
  const [stageScheduleByDiv,       setStageScheduleByDiv]       = useState({ boys: [], girls: [] });
  const [stageScheduleSaving, setStageScheduleSaving] = useState({ boys: false, girls: false });
  ...
  /* ── Teams state (single fetch, both divisions tagged — filtered per-section at render) ── */
  ...
  const loadFixtures = (eventId) => {
    api.get(`/events/${eventId}/fixtures`)
      .then(d => {
        ...
        /* Reconstruct date-groups from flat list, one set per division */
        const buildGroups = (division) => {
          const groups = [];
          const byDate = {};
          flat.filter(fix => (fix.division || 'boys') === division).forEach(fix => {
            const key = fix.date || '';
            if (!byDate[key]) {
        ...
        setFixturesByDiv({ boys: buildGroups('boys'), girls: buildGroups('girls') });
      })
      .catch(() => { setFixturesByDiv({ boys: [], girls: [] }); setFlatFixtures([]); });
  };

  const loadStageSchedule = (eventId, division) => {
    api.get(`/events/${eventId}/stage-schedule?division=${division}`)
      .then(d => setStageScheduleByDiv(p => ({ ...p, [division]: d.stages || [] })))
      .catch(() => setStageScheduleByDiv(p => ({ ...p, [division]: [] })));
  };

  const loadEventLiveScores = (eventId) => {
    if (!club) return;
  ...
  const viewRegs = (ev) => {
    ...
    setRegs([]);
    setTeams([]);
    setFixturesByDiv({ boys: [], girls: [] });
    setFlatFixtures([]);
    setStageScheduleByDiv({ boys: [], girls: [] });
    setScoreInputs({});
    setWinnerInputs({});
    ...
    loadFixtures(ev._id);
    loadStageSchedule(ev._id, 'boys');
    loadStageSchedule(ev._id, 'girls');
  };
  ...
  useEffect(() => {
    ...
    /* A later-round fixture/scoreboard was auto-created (or retracted) server-side —
       re-fetch since a brand-new row can't be patched into existing state. */
    const onAdvanced = ({ eventId }) => {
      if (String(regEvent._id) !== String(eventId)) return;
  ...
  /* ── Fixture helpers (date-grouped structure, per division) ──
     fixturesByDiv[division] = [{ date, venue, matches: [{teamA,teamB,time,round}] }]  */
  const addDateGroup = (division) =>
    setFixturesByDiv(p => ({ ...p, [division]: [...p[division], { date: '', venue: '', matches: [{ teamA: '', teamB: '', time: '', round: '' }] }] }));

  const addMatchToGroup = (division, gi) =>
    setFixturesByDiv(p => ({ ...p, [division]: p[division].map((g, i) => i === gi
      ? { ...g, matches: [...g.matches, { teamA: '', teamB: '', time: '', round: '' }] }
      : g) }));

  const updateGroup = (division, gi, key, val) =>
    setFixturesByDiv(p => ({ ...p, [division]: p[division].map((g, i) => i === gi ? { ...g, [key]: val } : g) }));

  const updateMatch = (division, gi, mi, key, val) =>
    setFixturesByDiv(p => ({ ...p, [division]: p[division].map((g, i) => i === gi
      ? { ...g, matches: g.matches.map((m, j) => j === mi ? { ...m, [key]: val } : m) }
      : g) }));

  const removeMatch = (division, gi, mi) =>
    setFixturesByDiv(p => ({ ...p, [division]: p[division].map((g, i) => i === gi
      ? { ...g, matches: g.matches.filter((_, j) => j !== mi) }
      : g).filter(g => g.matches.length > 0) }));

  const removeDateGroup = (division, gi) =>
    setFixturesByDiv(p => ({ ...p, [division]: p[division].filter((_, i) => i !== gi) }));

  const handleSaveDeclare = async (division) => {
    setDeclareLoading(p => ({ ...p, [division]: true }));
    try {
      ...
      setDeclareLoading(p => ({ ...p, [division]: false }));
    }
  };

  /* ── Stage schedule (estimated date/venue per later auto-advanced round), per division ── */
  const updateStageField = (division, label, key, val) =>
    setStageScheduleByDiv(prev => ({ ...prev, [division]: prev[division].map(s => s.label === label ? { ...s, [key]: val } : s) }));

  const handleSaveStageSchedule = async (division) => {
    setStageScheduleSaving(p => ({ ...p, [division]: true }));
    try {
      await api.put(`/events/${regEvent._id}/stage-schedule`, { stages: stageScheduleByDiv[division], division });
      showToast(`${DIVISION_LABEL[division]} stage dates saved.`);
    } catch (err) {
      showToast(err.message || 'Failed to save stage dates.', 'err');
    } finally {
      setStageScheduleSaving(p => ({ ...p, [division]: false }));
    }
  };
  ...
  const applyResult = (fixtureId, scoreA, scoreB, winner) => {
    ...
    setFixturesByDiv(prev => ({
      boys:  prev.boys.map(g  => ({ ...g, matches: g.matches.map(update) })),
      girls: prev.girls.map(g => ({ ...g, matches: g.matches.map(update) })),
    }));
  };
  ...
  const goLive = async (match) => {
    const tA = teams.find(t => t.name === match.teamA && t.division === match.division);
    const tB = teams.find(t => t.name === match.teamB && t.division === match.division);
  ...
  /* ── Group actions ── */
  const handleCreateGroup = async (division) => {
    try {
      const d = await api.post(`/events/${regEvent._id}/groups`, { division });
  ...
  const groupsByDiv = {
    ...
    girls: groups.filter(g => g.division === 'girls'),
  };
  const flatFixturesByDiv = {
    boys:  flatFixtures.filter(f => (f.division || 'boys') !== 'girls'),
    girls: flatFixtures.filter(f => f.division === 'girls'),
  };
  const liveScoresByDiv = {
    boys:  eventLiveScores.filter(s => (s.division || 'boys') !== 'girls'),
    girls: eventLiveScores.filter(s => s.division === 'girls'),
  };
  const unassignedTeamsByDiv = {
    boys:  teamsByDiv.boys.filter(t => !assignedTeamIds.has(t.id)),
    girls: teamsByDiv.girls.filter(t => !assignedTeamIds.has(t.id)),
  };
  ...
  const handleCreateTeam = async (division) => {
    ...
    try {
      const { team } = await api.post(`/events/${regEvent._id}/teams`, {
        name: name.trim(),
        maxSize: Number(newTeamSize[division]) || 0,
        division,
      });
      setTeams(p => [...p, team]);
            ...
            {fcQueue.map(req => {
              ...
              return (
                <div key={req.id} className={es.reqCard}>
                  ...
                  <ReadMore className={es.reqDesc} text={req.description} limit={140} />
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              {displayedEvents.map(ev => {
                ...
                <div key={ev._id} className={s.card}>
                  ...
                  {ev.description && (
                    <ReadMore className={s.desc} text={ev.description} limit={120} />
                  )}
          ...
          <div className={es.regsModal} onClick={e => e.stopPropagation()}>
            ...
            {regsTab === 'groups' && (
              <div className={es.groupsPanel}>
                {groupsLoading ? (
                  ...
                  const divGroups = groupsByDiv[division];
                  const divUnassigned = unassignedTeamsByDiv[division];
                  return (
                    <div key={division} className={es.divisionSection}>
                      <div className={es.divisionSectionTitle}>{DIVISION_LABEL[division]}</div>
                      ...
                      <div className={es.groupsLayout}>
                        ...
                        <div className={es.groupsPool}>
                          ...
                          {teamsByDiv[division].length === 0 && (
                            <div className={es.groupsPoolEmpty}>Create {DIVISION_LABEL[division].toLowerCase()} teams first in the Teams tab.</div>
                          )}
                        </div>
                      ...
                      <div className={es.groupsActions}>
                        <button className={es.addFixtureBtn} onClick={() => handleCreateGroup(division)}>
                          + Add Group
                        </button>
                      ...
                      {divGroups.length >= 1 && (
                        <div className={es.bracketPreview}>
                          <div className={es.bracketPreviewTitle}>{DIVISION_LABEL[division]} Bracket Preview</div>
                          <TournamentBracket groups={divGroups} fixtures={flatFixturesByDiv[division]} />
                        </div>
                      )}
                    </div>
                  );
            ...
            {regsTab === 'fixtures' && (
              <div className={es.fixturesPanel}>
                ...
                {DIVISIONS.map(division => {
                  ...
                  const divFixtures  = fixturesByDiv[division];
                  const divStages    = stageScheduleByDiv[division];
                  const divDeclaredAlready = flatFixturesByDiv[division].length > 0;
                  return (
                    <div key={division} className={es.divisionSection}>
                      <div className={es.divisionSectionTitle}>{DIVISION_LABEL[division]}</div>
                      ...
                      <div className={es.fixturesList}>
                        {divFixtures.length === 0 && (
                          <div className={es.regsEmpty}>
                            <p>No {DIVISION_LABEL[division].toLowerCase()} fixtures yet. Add a date to start scheduling.</p>
                          </div>
                        )}

                        {divFixtures.map((group, gi) => (
                              ...
                              <div className={es.fixtureDateGroupFields}>
                                <div className={es.fixtureMetaField}>
                                  ...
                                  <input
                                    ...
                                    value={group.date}
                                    onChange={e => updateGroup(division, gi, 'date', e.target.value)} />
                                </div>
                                <div className={es.fixtureMetaField} style={{ flex: 2 }}>
                                  ...
                                  <input
                                    ...
                                    value={group.venue}
                                    onChange={e => updateGroup(division, gi, 'venue', e.target.value)} />
                                </div>
                              </div>
                              <button className={es.fixtureRemoveBtn} onClick={() => removeDateGroup(division, gi)} title="Remove this date">✕</button>
                            </div>
                              ...
                              {group.matches.map((m, mi) => (
                                <div key={mi} className={es.fixtureMatchLine}>
                                  <select
                                    ...
                                    value={m.teamA}
                                    onChange={e => updateMatch(division, gi, mi, 'teamA', e.target.value)}>
                                    <option value="">Team A…</option>
                                    {divTeams.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
                                  </select>
                                  <span className={es.fixtureVsLabel}>vs</span>
                                  <select
                                    className={es.fixtureSelect}
                                    value={m.teamB}
                                    onChange={e => updateMatch(division, gi, mi, 'teamB', e.target.value)}>
                                  ...
                                  <input
                                    ...
                                    value={m.time}
                                    onChange={e => updateMatch(division, gi, mi, 'time', e.target.value)} />
                                  <input
                                    className={es.fixtureInput}
                                    style={{ maxWidth: 140 }}
                                    placeholder="Round / Stage"
                                    value={m.round}
                                    onChange={e => updateMatch(division, gi, mi, 'round', e.target.value)} />
                                  {m.autoGenerated && (
                                    <span className={es.declaredBadge} title="Created automatically once both teams advanced">Auto</span>
                                  )}
                                  <button className={es.fixtureRemoveBtn} onClick={() => removeMatch(division, gi, mi)} title="Remove match">✕</button>
                                </div>
                              ))}
                              <button className={es.addMatchInGroupBtn} onClick={() => addMatchToGroup(division, gi)} disabled={divTeams.length < 2}>
                                + Add Match
                              </button>
                            </div>
                          </div>
                      ...
                      <div className={es.fixtureActions}>
                        <button className={es.addFixtureBtn} onClick={() => addDateGroup(division)} disabled={divTeams.length < 2}>
                          + Add Date
                        </button>
                        {divTeams.length < 2 && (
                          <span className={es.fixtureHint}>Create at least 2 {DIVISION_LABEL[division].toLowerCase()} teams first.</span>
                        )}
                        <button
                          className={es.saveDeclareBtn}
                          onClick={() => handleSaveDeclare(division)}
                          disabled={declareLoading[division] || divFixtures.length === 0}>
                          {declareLoading[division] ? 'Saving…' : divDeclaredAlready ? '✓ Update & Re-declare' : 'Save & Declare'}
                        </button>
                      </div>

                      {divStages.length > 0 && (
                        <div className={es.fixtureDateGroup} style={{ marginTop: 18 }}>
                          <div className={es.fixtureDateGroupHead}>
                            <div className={es.fixtureMetaField}>
                              <label className={es.fixtureLbl} style={{ fontWeight: 700 }}>Stage Schedule</label>
                              <span className={es.fixtureHint}>
                                Later rounds are created automatically as winners advance — just give each stage an estimated date/venue.
                              </span>
                            </div>
                          </div>
                          <div className={es.fixtureMatchesBlock}>
                            {divStages.map(stage => (
                              <div key={stage.label} className={es.fixtureMatchLine}>
                                <span className={es.matchRoundLabel} style={{ minWidth: 110 }}>{stage.label}</span>
                                <input
                                  type="date"
                                  className={es.fixtureInput}
                                  value={stage.date}
                                  onChange={e => updateStageField(division, stage.label, 'date', e.target.value)} />
                                <input
                                  className={es.fixtureInput}
                                  style={{ maxWidth: 110 }}
                                  type="text"
                                  placeholder="Time e.g. 4:00 PM"
                                  value={stage.time}
                                  onChange={e => updateStageField(division, stage.label, 'time', e.target.value)} />
                                <input
                                  className={es.fixtureInput}
                                  placeholder="Venue"
                                  value={stage.venue}
                                  onChange={e => updateStageField(division, stage.label, 'venue', e.target.value)} />
                              </div>
                            ))}
                          </div>
                          <div className={es.fixtureActions}>
                            <button
                              className={es.saveDeclareBtn}
                              onClick={() => handleSaveStageSchedule(division)}
                              disabled={stageScheduleSaving[division]}>
                              {stageScheduleSaving[division] ? 'Saving…' : 'Save Stage Dates'}
                            </button>
                          </div>
                        </div>
                      )}
            ...
            {regsTab === 'scoreboard' && (
              <div className={es.scoreboardPanel}>
                ...
                {DIVISIONS.map(division => {
                  const divFlatFixtures = flatFixturesByDiv[division];
                  const divLiveScores   = liveScoresByDiv[division];
                  const divTeams        = teamsByDiv[division];
                  return (
                  <div key={division} className={es.divisionSection}>
                    <div className={es.divisionSectionTitle}>{DIVISION_LABEL[division]}</div>

                {divFlatFixtures.length === 0 ? (
                  <div className={es.scoreboardEmpty}>
                    <div style={{ fontSize: '2rem', marginBottom: 8 }}>🏟️</div>
                    <p>No {DIVISION_LABEL[division].toLowerCase()} fixtures yet. Add and declare fixtures in the Fixtures tab first.</p>
                  </div>
                ) : (
                  /* Group by date — each date gets a header + match list */
                  [...new Set(divFlatFixtures.map(f => f.date || '').filter(Boolean))].sort().map(date => {
                    ...
                    <div key={date} className={es.gamedaySection}>
                      <div className={es.gamedayBar}>
                        ...
                        <span className={es.gamedayMatchCount}>
                          {divFlatFixtures.filter(f => f.date === date).length} match(es)
                        </span>
                        {divLiveScores.some(s => s.status === 'live' && divFlatFixtures.filter(f => f.date === date).some(m => m.id === s.fixtureId)) && (
                      ...
                      <div className={es.matchCardList}>
                        {divFlatFixtures.filter(f => f.date === date).map(match => {
                      const teamA      = divTeams.find(t => t.name === match.teamA);
                      const teamB      = divTeams.find(t => t.name === match.teamB);
                      ...
                      const ls         = divLiveScores.find(s => s.fixtureId === match.id);
                ...
                {(() => {
                  const fixtureIds = new Set(divFlatFixtures.map(f => f.id));
                  const orphans = divLiveScores.filter(s => s.fixtureId && !fixtureIds.has(s.fixtureId));
                ...
                {teamsLoading ? (
                  ...
                  <div key={division} className={es.divisionSection}>
                    ...
                    {divTeams.length === 0 ? (
                      <div className={es.regsEmpty}>
                        <div style={{ fontSize:'2rem', marginBottom:8 }}>👥</div>
                        <p>No {DIVISION_LABEL[division].toLowerCase()} teams yet.</p>
